...
def migrate_month(
    ...
    for day in sorted(day_groups.keys()):
        ...
        # 冪等性チェック: 既に TAR が存在する場合はスキップ
        if day in tar_exists:
            logger.info(f"[Skip] {year}-{month}-{day}: TAR が既に存在します ({len(files)} files)")
            skipped += 1
            continue
        ...
        try:
            ...
            logger.info(f"  TAR 作成: {local_tar.stat().st_size:,} bytes, {len(local_files)} members")
