...
class MergerEngine:
    ...
    def _log_resources(label: str):
        """【診断用】メモリ・ディスクの使用状況を出力する (標準ライブラリのみ)"""
        try:
            # メモリ: /proc/self/status から VmRSS を読む (Linux)
            rss_mb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024  # KB → MB (Linux)
            # ディスク
    ...
    def run(self) -> bool:
        ...
        self._log_resources("RAWアップロード完了")
    ...
    def _upload_raw_files(self):
        ...
        self._log_resources("_upload_raw_files 開始")
        ...
        self._log_resources("Gatekeeper完了")
